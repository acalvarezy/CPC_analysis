...
os.chdir('/Users/catalinaalvarez/Documents/CPC_plots_2026/simulations_comparison/tensed_vs_relaxed/')

#CODE 2: SIMULATIONS COMPARISON - Simultaneous plotting of 2 chromosome regions (IC or KT + BG)
...
#     if os.path.isdir(f"/Users/catalinaalvarez/Documents/CPC_plots_2026/simulations_comparison/tensed_vs_relaxed/{name_folder}"):
#         pass
#     else:
#         os.makedirs(f"/Users/catalinaalvarez/Documents/CPC_plots_2026/simulations_comparison/tensed_vs_relaxed/{name_folder}")
...
#                 print("saving fig")
#                 print(tag)
#                 plt.savefig(f"/Users/catalinaalvarez/Documents/CPC_plots_2026/simulations_comparison/tensed_vs_relaxed/{name_plot}_{tag}%_kt_bg_500s.pdf")
#             tag = tag + 5
#             plt.show()
#             plt.close()
...
#CODE 2: SIMULATIONS COMPARISON - individual plotting of background-corrected regions (IC or KT)
def plot_across_models(species, plot_list, in_dir, location, name_list = [], column = "Sum_Active", active = 'active',
                        name = None, name_plot="", name_folder =""):
    print("Plotting across models")
    if os.path.isdir(f"/Users/catalinaalvarez/Documents/CPC_plots_2026/simulations_comparison/tensed_vs_relaxed/{name_folder}"):
    ...
    if active == 'all':
        ...
        for i in range(0, len(plot_list), 2):            
            ...
            for n, p in zip(current_names,current_plots):
                ...
                result = tmpc.loc[tmpc['Time'] == 200, 'bg_corrected'].values[0]
                print(result)
                df.loc[len(df)] = [n, result]
                plot_data = pd.concat([plot_data,tmpc[['parameter','bg_corrected', 'Time']]], ignore_index=True)
                column = 'bg_corrected'
                plot_data = pd.concat([plot_data,tmpc[['parameter',column, 'Time']]], ignore_index=True)     
            print(df)    
            ratio = round(df.iloc[0, 1] / df.iloc[1, 1],3)
            print(ratio)
            fig = plt.figure(figsize = (4,3))
            ax = sns.lineplot(x = plot_data['Time'], y= plot_data[column], hue = plot_data['parameter'], palette="magma")
            # hue = plot_data['parameter'].to_numpy()  --> palette = "crest"
            ...
            #Change legend labels here following simulation order:
                L=ax.legend()
                L.get_texts()[0].set_text('Relaxed bg corrected')
                L.get_texts()[1].set_text('Tensed bg corrected')
                ...
                print("saving fig")
                print(tag)
                plt.savefig(f"/Users/catalinaalvarez/Documents/CPC_plots_2026/simulations_comparison/tensed_vs_relaxed/{name_plot}_{tag}%_ic_corrected_500s.pdf")
            tag = tag + 5
            plt.show()
            plt.close()
