...
def main():
    ...
    for i in range(n_panels):
        ...
        x2a, y2a = data_2a[i]
        ax.plot(x2a, y2a, color=RED, linewidth=2.2)
        ax.set_xlim(0, X_DATA_MAX)
        ax.set_ylim(Y_VALUES_2A[-1], Y_VALUES_2A[0])
        ...
        ax2 = ax.twinx()
        x3, y3 = data_3[i]
        ax2.plot(x3, y3, color=ORANGE, linewidth=2.2)
        ax2.set_ylim(Y_VALUES_3[-1], Y_VALUES_3[0])

        if i == 0:
            ax.set_yticks(sorted(Y_VALUES_2A))
            ax.tick_params(axis="y", labelsize=7.5, colors=RED)
            ax.set_ylabel("[p-H2A] (µM)", fontsize=9, color=RED)
        else:
            ax.set_yticks([])

        if i == n_panels - 1:
            ax2.set_yticks(sorted(Y_VALUES_3))
            ax2.tick_params(axis="y", labelsize=7.5, colors=ORANGE)
            ax2.set_ylabel("p-H3 (µM)", fontsize=9, color=ORANGE, rotation=270, labelpad=14)
        else:
            ax2.set_yticks([])
