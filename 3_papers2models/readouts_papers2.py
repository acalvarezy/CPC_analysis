...
fig, ax_top = plt.subplots(figsize=(5, 3.5)) # Adjusted height slightly for multi-lane labels
sns.set_theme(style="ticks")
# ax_top = fig.add_axes([0.50, 0.15, 0.20, 0.65])

# --- COORDINATED LAYOUT TRACKS ---
reporter_offset = 0.275
lit_offset = 0.00
model_offset = -0.275
...
for _, row in lit_summary.iterrows():
    ax_top.text(
        ...
        fontsize=16, fontweight="bold", color="black", ha="right", va="center"
    )
    ax_top.text(
        x=-2, y=row["y_pos"] + lit_offset, s=f"PMID: {row['pubmedID']}",
        fontsize=14, color="#2b5c8f", ha="right", va="center"
    )
    # ax_top.text(
    #     x=-2, y=row["y_pos"] + model_offset, s="Model prediction",
    #     fontsize=14, fontweight="semibold", color="#d95f02", ha="right", va="center"
    # )
...
ax_top.set_xlabel("Phosphorylation stoichiometry", fontsize=16, labelpad=10)
...
plt.savefig(
    "/Users/catalinaalvarez/Google Drive/My Drive/UVA/Research/JanesLab/CPC_project/CPC PAPER/Supplementary material/Experimental support/Readouts_celllines2.pdf",
    format="pdf",
    bbox_inches="tight",
    dpi=300
)
