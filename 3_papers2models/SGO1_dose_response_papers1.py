...
# Extract just ONE single color (index 1 is the dark orange) from the palette list
...
valid_line_mask = np.isfinite(y_fc_log2)
ax.plot(x_conditions, y_fc_log2, linestyle='-', color='black', linewidth=1.2)

# Main data points with error bars
ax.errorbar(x_indices, y_fc_log2, yerr=asymmetric_yerr, fmt='o', color=seaborn_dark_orange, 
            ecolor=seaborn_dark_orange, elinewidth=1.2, capsize=4, mfc=seaborn_dark_orange, 
            mec=seaborn_dark_orange, ms=14, zorder=5, label='Reported data')


# 8. APPLY CUSTOM AXIS & GRID FORMATTING
ax.set_xticks(ticks=x_indices)
ax.set_xticklabels(labels=x_conditions, fontsize=14, fontweight='bold') 
...
for idx, pmid in zip(x_indices, x_pmids):
    ax.text(idx, -0.11, f"(PMID: {pmid})", 
            fontsize=12, color='dimgray', ha='center', va='top', transform=ax.get_xaxis_transform())
...
plt.tight_layout()
plt.savefig("/Users/catalinaalvarez/Google Drive/My Drive/UVA/Research/JanesLab/CPC_project/CPC PAPER/Supplementary material/Experimental support/SGO1_papers_final.pdf", format="pdf", bbox_inches="tight", dpi=300)
plt.show()